 ...
 */
function solveLinearEquation(a: number, b: number) {
    if (a == 0)
        if (b != 0)
            return [];
        else
...
function solveQuadraticEquation(a: number, b: number, c: number) {
    ...
    if (sqrtContent < 0) {
        ...
        return [new Complex(-p / 2, sqrtR), new Complex(-p / 2, -sqrtR)];
    } else {
        const sqrtR = Math.sqrt(sqrtContent);
...
function calcX(d0: number, c: Complex, a: number, b: number): Complex {
    ...
    const res = new Complex(-(dDivc.re + c.re + b) / (3 * a), -(dDivc.im + c.im) / (3 * a));
    return res;
}
 ...
 */
function solveCubicEquation(a: number, b: number, c: number, d: number) {
    if (a == 0)
        return solveQuadraticEquation(b, c, d);

    if (d == 0) {
        const x = solveQuadraticEquation(a, b, c);
        const y: (number | Complex | undefined)[] = [0];
        const l = x.length;
